"""
Import 5 companies - V7 TELCO CONTACTS TABLE

V7 IMPROVEMENTS:
- Uses telco.contacts table (pre-aggregated stats)
- Uses telco.normalize_phone() for proper phone matching
- Queries ALL available phones: Excel, Appointment, Company
- Captures is_dnc, contact_status, lead_score from contacts table
...
# === TELCO WAREHOUSE QUERIES (V7 - Using telco.contacts) ===
...
def build_telco_attributes_v7(contact_data, call_entries):
    """
    Build Brevo attributes from telco.contacts data and call entries.
    Uses pre-aggregated stats from contacts table.
    """
    attrs = {}
    ...
    if contact_data:
        ...
        if contact_data.get('last_transcript'):
            transcript = contact_data['last_transcript']
            if len(transcript) > 10000:
                transcript = transcript[:9990] + '...[truncated]'
            attrs['RETELL_TRANSCRIPT'] = transcript

        if contact_data.get('last_disposition'):
            attrs['TELCO_LAST_DISPOSITION'] = contact_data['last_disposition']
...
def create_contact(appt_data, company_id, company_name, source_type,
    ...
    attributes['IMPORT_BATCH'] = "v7_telco_contacts"
...
def main():
    print("=" * 70)
    print("IMPORT 5 COMPANIES - V7 (TELCO.CONTACTS TABLE)")
    print("=" * 70)
    print()
    print("V7 Improvements:")
    print("  + Uses telco.contacts table (pre-aggregated stats)")
    print("  + Uses telco.normalize_phone() for proper matching")
    print("  + Queries ALL phones: Excel, Appointment, AND Company")
    print("  + Captures is_dnc, contact_status, lead_score")
    print()
    ...
    for company_name in TARGETS:
        ...
        # V7: Get call data from telco.contacts using ALL available phones
        telco_attrs = {}
        ...
        if telco_conn and phones_to_try:
            ...
            if telco_contact:
                telco_attrs = build_telco_attributes_v7(telco_contact, call_entries)
                ...
                print(f"    Telnyx: {telco_contact.get('telnyx_calls', 0)}")
            else:
    ...
    for c in results['contacts']:
        ...
        if c.get('matched_phone'):
            extras.append(f"Matched: {c['matched_phone']}")
    ...
    # Save results
    results_file = Path(__file__).parent / 'import_v7_results.json'
